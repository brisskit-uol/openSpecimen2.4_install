...
public class caTissue_BarcodeOntology {
	...
	public static void main(String ar[])
	{
		try
		{
			new caTissue_BarcodeOntology().createEnumVariables();
	...
	public void createEnumVariables() throws Exception {
		try {
			pstmt1 = (PreparedStatement) getSimpleConnectionSQL().prepareStatement(
					"select distinct cas.Specimen_Class,replace(cas.SPECIMEN_TYPE,' ',''),replace(csn.BARCODE,' ','_') " +
			...
			String spec_class="", spec_type="", barcode="";
	...
	private void createXmlTree(String specimen, HashMap<String, ArrayList<String>> specMap)
	{
		...
		try
		{
			...
			File file = new File("/var/local/brisskit/openspecimen/system/OpenSpecimen-2.4.RC2/tomcat-as/webapps/integration/temp/files/barcode."+spec_class+"."+spec_type+"_"+new SimpleDateFormat( "ddMMyy" ).format(new Date())+".xml");
