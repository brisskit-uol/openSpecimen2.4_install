...
public class caTissue_ProtocolOntology {
	...
	public static void main(String ar[])
	{
		try
		{
			new caTissue_ProtocolOntology().createEnumVariables();
	...
	public void createEnumVariables() throws Exception {
		try {
			pstmt1 = (PreparedStatement) getSimpleConnectionSQL().prepareStatement(
					"select distinct cas.Specimen_Class,replace(cas.SPECIMEN_TYPE,' ',''),replace(cpr.TITLE,' ','_') " +
			...
			String spec_class="", spec_type="", protocol="";
	...
	private void createXmlTree(String specimen, HashMap<String, ArrayList<String>> specMap)
	{
		...
		try
		{
			...
			File file = new File("/var/local/brisskit/openspecimen/system/OpenSpecimen-2.4.RC2/tomcat-as/webapps/integration/temp/files/protocol."+spec_class+"."+spec_type+"_"+new SimpleDateFormat( "ddMMyy" ).format(new Date())+".xml");
