...
public class caTissue_StorageOntology {
	...
	public static void main(String ar[])
	{
		try
		{
			new caTissue_StorageOntology().createEnumVariables();
	...
	public void createEnumVariables() throws Exception {
		try {
			pstmt1 = (PreparedStatement) getSimpleConnectionSQL().prepareStatement(
					"select distinct cas.Specimen_Class,replace(cas.SPECIMEN_TYPE,' ',''),csn.LABEL,csn.BARCODE " +
			...
			String spec_class="", spec_type="", storage="", hospital="";
				...
				while (rs1.next() ) {
					pstmt2 = (PreparedStatement) getSimpleConnectionSQL().prepareStatement(
							"select cc.name,cap.POSITION_DIMENSION_ONE,cap.POSITION_DIMENSION_TWO "+
									...
									"and csn.LABEL='"+rs1.getString(3)+"' and csn.BARCODE='"+rs1.getString(4)+"'");
					pstmt2.execute();
					...
					if(rs2.next())
					{
						hospital=""+rs2.getString(1).charAt(0)+rs2.getString(1).charAt(rs2.getString(1).indexOf(" ")+1)+rs2.getString(1).substring(rs2.getString(1).indexOf("_"));
						storage = hospital+"-"+rs2.getString(2)+":"+rs2.getString(3);
						list.add(storage);
					}
	...
	{
		String spec_class="", spec_type="", storage="";
		...
		try
		{
			...
			File file = new File("/var/local/brisskit/openspecimen/system/OpenSpecimen-2.4.RC2/tomcat-as/webapps/integration/temp/files/storage."+spec_class+"."+spec_type+"_"+new SimpleDateFormat( "ddMMyy" ).format(new Date())+".xml");
